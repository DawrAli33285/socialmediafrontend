...
if (error) {
   toastr.error(error)
    ...
    console.log('state')
    console.log(subdata)
 let res=await dispatch(subscription(subdata))
    // You can now use the paymentMethod.id to create a charge or a customer
    //newcomments
  if(subscription.rejected.match(res)){
  ...
  if(subscription.fulfilled.match(res)){
...
100% sécurisé
      ...
      <div className='payments-center-div'>
        <p className='my-2'>
        Abonnement à  {state?.creatorname}
        </p>
      </div>
          ...
          <div className="payments-form p-8 rounded-lg shadow-lg w-full max-w-md">
            ...
            <div className="mb-4">
              <div className="flex justify-between monthly-sub-status items-center">
                <span className="text-gray-600">Abonnement d'appel mensuel {state?.creatorname}</span>
...
En finalisant mon règlement, j'atteste mon accord avec les CGV et renonce à tout recours au droit de rétractation
            ...
            Abonnement d'appel mensuel de {state.price} / mois - annulable a tout moment
            </p>
