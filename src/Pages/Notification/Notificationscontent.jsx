...
const Notificationscontent = () => {
  ...
  React.useEffect(() => {
    const socket = socketcontext?.current;

    const handleSocketNotification = (data) => {
      console.log('socketnotification received', data);
    ...
    if (socket) {
      socket.on('socketnotification', handleSocketNotification);

      // Cleanup function to remove the listener
      return () => {
        socket.off('socketnotification', handleSocketNotification);
      };
    }
  ...
  const fetchNotification = async () => {
    ...
    if (getNotifications.rejected.match(res)) {
      console.log('notificationget response', res);
  ...
  return (
    <div className="notificationscontent-div">
      {notifications?.length > 0 ? notifications.map((val, i) => {
        console.log('notification', val);
        return (
          <div key={i.toString()} className="notification-data flex flex-row relative">
            <div className="flex flex-row space-x-2 notification-avatar-text">
              {val?.creator ? (
                <div className="notification-avatar">
                  <img src={val?.creator ? "https://cdn.pixabay.com/photo/2014/04/03/10/32/user-310807_1280.png" : ''} alt="avatar" />
                </div>
              ) : ''}
